...
درک نابالغ ما از این سه عنصر و پیامدهای ناکارآمد ناشی از آن همچنان ادامه خواهد داشت تا زمانی که یک ساختار موثر برای تحلیل (یک چارچوب متنی، که به نوعی با یک چارچوب مفهومی قابل مقایسه است) برای این سه عنصر وجود داشته باشد - یعنی یک چارچوب متنی که تحقیق و تحلیل را فراهم می‌کند و منجر به توسعه کاربردهایی می‌شود که در تولید تأثیر مثبت بر رفتار موثر هستند.

بحران گسترده عمومی، تعداد زیادی از کتاب‌ها و مقالات، و گفتمان فلسفی طولانی مدت در مورد این سه عنصر، هم اهمیتی که ما بر آن‌ها قائل هستیم و هم نبود وضوح موجود که برای ایجاد نتایج مطلوب در زندگی مردم ضروری است، را نشان می‌دهد. این موضوع به نقش و تعاملات افراد در خانواده‌ها، سازمان‌ها، جوامع، اجتماعات و ملل متحد می‌پردازد.

فقط حل کردن سردرگمی و اشتباه بین اصطلاحات در داخل همان ساختار قدیمی برای تحلیل (چارچوب متنی) **قدرت**[^1] پتانسیل نادیده گرفته و بنابراین استفاده نشده از این پدیده‌ها را آشکار نخواهد کرد. یکی از اهداف ما در این مدل جدید ارائه یک چارچوب متنی است که واقعاً قدرت پتانسیل این پدیده‌ها را آشکار کرده و دسترسی به آن فراهم می‌کند، و در داخل آن چارچوب متنی، وضوحی ارائه می‌دهد که سردرگمی و اشتباه بین آن‌ها را از بین می‌برد.

#### ب.۱. سه دیدگاه استفاده شده در چارچوب متنی ما
چارچوب متنی ما از سه دیدگاه یا جنبه متمایز نسبت به چهار عنصر مورد بررسی در این مدل جدید (تمامیت، اخلاقیات، اصول اخلاقی، قوانین) استفاده می‌کند. به عنوان مثال، "تمامیت" می‌تواند به سه شکل مورد بررسی قرار گیرد:

1. به عنوان یک پدیده
2. به عنوان یک مفهوم
3. به عنوان یک اصطلاح

با ادامه دادن به "تمامیت" به عنوان یک مثال از هر یک از چهار عنصر، زمانی که "تمامیت" به عنوان یک پدیده مورد بررسی قرار می‌گیرد، با آن به عنوان یک مورد واقعی یا نمونه بررسی می‌شود، یا با استناد به { mark \ تعریف پدیده در واژه‌نامه وبستر: “هر اتفاق، شرایط یا تجربه‌ای که برای **حواس**، آشکار و قابل رویت است و می‌تواند از نظر علمی، توصیف یا ارزیابی شود.” } در چارچوب متنی ما، همانطور که در زیر خواهیم دید، ما حوزه‌های مشخصی برای تمامیت، اخلاقیات، اصول اخلاقی و قوانین به عنوان پدیده‌ها ایجاد می‌کنیم.

زمانی که "تمامیت" به عنوان **یک مفهوم** مورد بررسی قرار می‌گیرد، با یک مورد واقعی از تمامیت سر و کار نداریم. به جای آن، با یک ایده کلی از تمامیت، کلاس یا حوزه‌ای که تمامیت به آن تعلق دارد، بررسی می‌شود. دوباره، همانطور که در زیر خواهیم دید، با گرفتن آنها به عنوان مفاهیم، ما حوزه‌های متمایز، غیر همپوشان از علاقه یا نگرانی برای هر یک از تمامیت، اخلاقیات، اصول اخلاقی و قوانین ایجاد می‌کنیم.

زمانی که "تمامیت" به عنوان **یک اصطلاح** مورد بررسی قرار می‌گیرد، نه با یک مورد واقعی از تمامیت و نه با حوزه تمامیت سر و کار داریم. به جای آن، با تعریفی از تمامیت سر و کار داریم. همانطور که در زیر خواهیم دید، با در نظر گرفتن آنها به عنوان اصطلاحات (در حالی که از تعاریف موجود واژه‌نامه استفاده می‌کنیم اما از جملاتی در تعاریف که همپوشانی دارند پرهیز می‌کنیم)، ما تعاریف منحصر به فردی برای هر یک از تمامیت، اخلاقیات، اصول اخلاقی و قوانین ایجاد می‌کنیم. بنابراین:

1. در چارچوب متنی ما، با اختصاص دادن هر یک از چهار پدیده به یکی از دو حوزه متمایز، یعنی یک حوزه مثبت بدون ارزش‌های هنجاری و یک حوزه هنجاری از فضیلت‌ها، شروع می‌کنیم. { mark \ در مدل جدید ما، تمامیت در حوزه مثبت وجود دارد، در حالی که اخلاقیات، اصول اخلاقی و قوانین در حوزه هنجاری از فضیلت‌ها وجود دارند. }
2. سپس، در حوزه‌های مربوط به خود، جنبه دوم چارچوب متنی ما با تشخیص حوزه‌های غیر همپوشان، متمایز و جداگانه برای هر یک از چهار مفهوم ایجاد می‌شود. ما { mark \ **حوزه اخلاقیات** را به عنوان یک حوزه فضیلت اجتماعی، حوزه **اصول اخلاقی** را به عنوان یک حوزه فضیلت گروهی و **حوزه قوانین** را به عنوان یک حوزه فضیلت دولتی تشخیص می‌دهیم. ما **حوزه تمامیت** را به عنوان وضعیت یا شرایط هدفمند یک شیء، سیستم، فرد، گروه، سازمان یا جامعه تشخیص می‌دهیم. }
3. و در نهایت، در حوزه‌های مربوط به خود، جنبه آخر چارچوب متنی ما با ارائه تعاریف منحصر به فرد برای هر یک از چهار اصطلاح ایجاد می‌شود تا هیچ گونه سردرگمی، اشتباه یا ابهامی بین آنها وجود نداشته باشد. ما این تعاریف را در بخش ۲ زیر ارائه می‌دهیم.

در خلاصه، چارچوب متنی ما سه دیدگاه یا جنبه دارد، به ویژه، یک حوزه برای هر عنصر به عنوان یک پدیده، یک حوزه برای هر عنصر به عنوان یک مفهوم، و یک تعریف برای هر عنصر به عنوان یک اصطلاح. بنابراین، ما به هر عنصر نگاه می‌کنیم به عنوان یک پدیده که در یک حوزه خاص وجود دارد، به عنوان یک مفهوم که یک حوزه مشخص از علاقه یا نگرانی را متمایز می‌کند، و به عنوان یک اصطلاح که به طور منحصر به فرد تعریف شده است.

این چارچوب متنی، که از سه جنبه فوق‌الذکر تشکیل شده است تا تمامیت، اخلاقیات، اصول اخلاقی و قوانین را بررسی کند، اولین عامل حیاتی در مدل جدید ما است.

به عنوان مثال، ما خواهیم دید که زمانی که تمامیت، اخلاقیات، اصول اخلاقی و قوانین، همانطور که در حال حاضر مورد نظر قرار می‌گیرند، از دیدگاه چارچوب متنی ما مورد بررسی قرار می‌گیرند:

1. به عنوان پدیده‌ها، همه چهار عنصر به حوزه هنجاری فرو می‌روند، و بدین ترتیب:
   - الف) قدرت تمامیت به عنوان بنیاد عملکرد که زمانی که تمامیت به عنوان یک پدیده مثبت مورد بررسی قرار می‌گیرد، آشکار می‌شود، از دست می‌رود، و
   - ب) قدرت تمامیت، زمانی که به عنوان یک پدیده مثبت مورد بررسی قرار می‌گیرد، برای افزایش تأثیر واقعی بر رفتار از پدیده‌های فضیلت هنجاری اخلاقیات، اصول اخلاقی و قوانین از دست می‌رود.
2. به عنوان مفاهیم، همپوشانی حوزه‌ها بین چهار عنصر، حوزه مشخص علاقه یا نگرانی که در هر یک مورد بررسی می‌شود، مبهم و نامشخص می‌کند و قدرت هر یک به عنوان یک مفهوم را ضعیف می‌کند، و
3. به عنوان اصطلاحات، استفاده از یکی از اصطلاحات در تعریف دیگری باعث سردرگمی و اشتباه بین همه چهار اصطلاح می‌شود.

در خلاصه، فرو رفتن همه چهار پدیده به حوزه هنجاری، و همپوشانی حوزه‌های هر یک به عنوان مفاهیم، و سردرگمی و اشتباه تعاریف هر یک به عنوان اصطلاحات، قدرت مخصوص به هر یک از عناصر را از دست می‌دهد. به طور جمعی، این منجر به تجزیه و تحلیل ناکارآمد و غیر دقیق، شعارهای بی‌محتوا، و سیاست‌ها و ارزش‌هایی می‌شود که اعلام می‌شوند اما رعایت نمی‌شوند.
...
---
[^1]: وقتی ما از عبارت "قدرت" استفاده می‌کنیم، منظور ما هر چیزی در یک طیف است، از توانمندی برای تولید نتایج گرفته تا مزیت رقابتی یا ایجاد ارزش، تا بهبود کیفیت زندگی. ما بین **قدرت** و **زور** تمایز قائل می‌شویم. و در روشی که ما قدرت را استفاده می‌کنیم، استفاده از زور منافی قدرت است، یعنی زور فرصت استفاده از قدرت را منتقل می‌کند.
