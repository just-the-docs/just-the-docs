...
`nav_fold: true` in `just_the-docs.collections.tests` *folds* the collection:
...
[^js-disabled]: <span>New (v0.6.0)</span>{: .label .label-green }
    When JavaScript is disabled in the browser, all collections are automatically expanded,
